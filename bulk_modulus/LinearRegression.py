...
# use linear regression to predict bulk modulus
lr = LinearRegression()
...
plt.title("Linear Regression Predicted Bulk Modulus")
...
plt.tight_layout()
plt.savefig("figs/Lr_predict.png", dpi=300)


# lets use Kfold cross validation
# Use 10-fold cross validation (90% training, 10% test)
kfv = KFold(n_splits=10, shuffle=True, random_state=1)
scores = cross_val_score(lr, X, y, cv=kfv, scoring='neg_mean_squared_error')
rmse_scores = [np.sqrt(abs(s)) for s in scores]
r2_scores = cross_val_score(lr, X, y, scoring='r2', cv=kfv, n_jobs=1)
